...
public class Digester extends DefaultHandler2 {
    ...
    public Rules getRules() {
        //当前 digester对象 存在一个 Map类型的rules 属性，她负责缓存当前项目的所有xml解析规则
        // 此处如果解析的规则属性 为null，那么给他初始化一个RulesBase
        if (this.rules == null) {
    ...
    public XMLReader getXMLReader() throws SAXException {
        ...
        //设置xml 的handle的处理解析类 为 当前对象，也就是说 当前对象实现有 startElement characters endElement  方法
        reader.setDTDHandler(this);
        //设置xml 的内容的处理解析类 为 当前对象
        reader.setContentHandler(this);

        EntityResolver entityResolver = getEntityResolver();
        if (entityResolver == null) {
            //当前digester拥有解析 xml 的规则
            entityResolver = this;
        }

        // Wrap the resolver so we can perform ${...} property replacement
        //这个地方就是如果 entityResolver 实现了EntityResolver2 接口
        if (entityResolver instanceof EntityResolver2) {
            //将entityResolver 生成一个包装类
        ...
        //将包装后的entityResolver放入 reader （给XMLReader放入解析器）
...
//        System.out.println(a+" "+b+" "+c);
    ...
    public Object parse(InputSource input) throws IOException, SAXException {
        ...
        //  小提示   ：parse,只是用来开始解析xml 配置文件的开始命令，并无特别含义，重点在getXMLReader中
        getXMLReader().parse(input);
        return root;
    }
    ...
    public void addRule(String pattern, Rule rule) {
        //给规则设置规则解析器 为当前对象，后期肯定会用来回调当前解析器的方法
